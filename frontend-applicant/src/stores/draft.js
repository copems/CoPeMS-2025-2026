...
// import draftService from "../services/draftService"; // Removed: backend service not used in frontend
...
export const useDraftStore = defineStore("draft", {
  ...
  actions: {
    ...
    async loadDrafts() {
      this.loading = true;
      try {
        // Backend call removed: getUserDrafts
        if (result.success) {
    ...
    async saveBuildingOwner(data) {
      ...
      try {
        // Backend call removed: saveBuildingOwnerDraft
        if (result.success) {
    ...
    async saveConstruction(data) {
      this.autoSaving = true;
      try {
        // Backend call removed: saveConstructionDraft
        if (result.success) {
    ...
    async saveProjectDetails(data) {
      this.autoSaving = true;
      try {
        // Backend call removed: saveProjectDetailsDraft
        if (result.success) {
    ...
    async saveEngineerInformation(data) {
      this.autoSaving = true;
      try {
        // Backend call removed: saveEngineerInformationDraft
        if (result.success) {
    ...
    async finalizeApplication() {
      this.loading = true;
      try {
        // Backend call removed: finalizeApplication
        if (result.success) {
          this.isDraft = false;
